...
class ActivityWatch {
  ...
  public init() {
    ...
    // Create AWClient
    this._client = new AWClient(this._bucket.clientName, {
  ...
  public loadConfigurations() {
    ...
    const maxHeartbeatsPerSec = extConfigurations.get<number>(
      "maxHeartbeatsPerSec"
    );
    ...
    const baseURL = extConfigurations.get<string>("baseURL");
    if (baseURL) {
      this._baseURL = baseURL;
    } else {
      // It is empty, so we set it to the user's requested value.
      this._baseURL = "http://192.168.1.3:5600";
    }

    const testing = extConfigurations.get<boolean>("testing");
    if (testing) {
